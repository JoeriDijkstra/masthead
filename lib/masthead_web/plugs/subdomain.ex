defmodule MastheadWeb.Plugs.Subdomain do
  @moduledoc """
  Resolves the subdomain on `conn.host` to a `Masthead.Sites.Site`.

  - On the bare app host (e.g. `masthead.local`, `localhost`, `lvh.me`), this is
  ...
  alias MastheadWeb.SiteNotFound
  ...
  defp extract_subdomain(host) when is_binary(host) do
    ...
    cond do
      ...
      true ->
        # `foo.lvh.me` -> "foo"; `bar.baz.masthead.local` -> "bar.baz" (we treat
