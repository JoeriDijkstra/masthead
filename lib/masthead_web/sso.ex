defmodule MastheadWeb.SSO do
  ...
  def buttons(assigns) do
    ...
    <div :if={@providers != []}>
      ...
      <div class="sso-buttons">
        <a :for={p <- @providers} class="sso-btn" href={~p"/auth/#{p.id}"}>
          <.provider_icon id={p.id} />
          <span>{p.label}</span>
        </a>
      </div>
    </div>
  ...
  defp provider_icon(%{id: :github} = assigns) do
    ~H"""
  ...
  defp provider_icon(%{id: :google} = assigns) do
    ~H"""
  ...
  defp provider_icon(assigns), do: ~H""
end
