defmodule MastheadWeb.SiteNotFound do
  @moduledoc """
  Renders the public "site not found" page. Used by
  `MastheadWeb.Plugs.Subdomain` when a request arrives on a subdomain that
  ...
  defp render(conn) do
    ...
    scheme =
      Application.get_env(:masthead, :site_url, [])
    ...
    <html lang="en">
      <head>
        ...
        <title>Site not found · Masthead</title>
        <style>#{style()}</style>
      </head>
      <body>
        <main class="card">
          ...
          <a class="btn" href="#{Plug.HTML.html_escape(home_url)}">&larr; Back to Masthead</a>
        </main>
      </body>
    </html>
    """
  ...
  defp primary_host do
    case Application.get_env(:masthead, :app_hosts, []) do
      [host | _] -> host
      _ -> "masthead.site"
    end
  end
