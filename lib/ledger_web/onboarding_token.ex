defmodule LedgerWeb.OnboardingToken do
  @moduledoc """
  ...
  def unsubscribe_url(user_id) do
    token = Phoenix.Token.sign(LedgerWeb.Endpoint, @salt, user_id)
    url(~p"/unsubscribe/onboarding/#{token}")
  end

  @doc "Verifies a token, returning `{:ok, user_id}` or `{:error, reason}`."
  def verify(token) do
    Phoenix.Token.verify(LedgerWeb.Endpoint, @salt, token, max_age: @max_age)
  end
end
