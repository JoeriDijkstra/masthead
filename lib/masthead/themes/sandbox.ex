defmodule Masthead.Themes.Sandbox do
  @moduledoc """
  ...
  consider safe for user-uploaded themes:
    ...
    * No file system — `{% include %}` and `{% render %}` (partials) are
      ...
      clear error.
    * Custom filter allowlist via `Masthead.Themes.Filters`. Solid's
  ...
  def render(%Template{} = template, context) when is_map(context) do
    Solid.render(template, context, custom_filters: Masthead.Themes.Filters)
  end
