defmodule Masthead.Storage.Adapter do
  @moduledoc """
  Behaviour for storage adapters. Two implementations ship with Masthead:

    * `Masthead.Storage.Local` — files on disk, served via `Plug.Static`
    * `Masthead.Storage.S3`    — S3-compatible storage (Tigris, R2, AWS, ...)

  Pick one by setting `config :masthead, Masthead.Storage, adapter: <Module>`.
  """
