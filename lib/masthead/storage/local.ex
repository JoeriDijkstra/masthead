defmodule Masthead.Storage.Local do
  @moduledoc """
  Local-disk storage adapter. Files live under `priv/uploads/` (overridable
  via `config :masthead, Masthead.Storage, root: "/some/path"`) and are served
  ...
  def root_path do
    Application.get_env(:masthead, Masthead.Storage)[:root] ||
      Path.join(:code.priv_dir(:masthead) |> to_string(), "uploads")
  end
end
