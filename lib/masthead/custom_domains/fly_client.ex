defmodule Masthead.CustomDomains.FlyClient do
  @moduledoc """
  ...
  def adapter do
    Application.get_env(
      :masthead,
      :fly_client,
      Masthead.CustomDomains.FlyClient.Http
    )
  end
...
defmodule Masthead.CustomDomains.FlyClient.Http do
  @moduledoc """
  Real Fly.io client. Talks to the Fly GraphQL API
  (`https://api.fly.io/graphql`). Requires:

    * `FLY_API_TOKEN` — a Fly API/deploy token
    * `FLY_APP_NAME`  — the app to attach certs to (e.g. `dijkstra-masthead`)
  """
  @behaviour Masthead.CustomDomains.FlyClient
...
defmodule Masthead.CustomDomains.FlyClient.Stub do
  @moduledoc """
  Test/dev Fly client. Behaviour is driven by application env:

      config :masthead, :fly_stub, %{add: :ok, status: "Ready", delete: :ok}
