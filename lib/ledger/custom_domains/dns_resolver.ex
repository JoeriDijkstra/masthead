defmodule Ledger.CustomDomains.DnsResolver do
  @moduledoc """
  ...
  def adapter do
    Application.get_env(
      :ledger,
      :dns_resolver,
      Ledger.CustomDomains.DnsResolver.Inet
    )
  end
...
defmodule Ledger.CustomDomains.DnsResolver.Inet do
  ...
  @behaviour Ledger.CustomDomains.DnsResolver
...
defmodule Ledger.CustomDomains.DnsResolver.Stub do
  ...
  Test/dev DNS resolver. Reads canned answers from application env:
      ...
      config :ledger, :dns_stub, %{
        txt: %{"_ledger-verify.blog.example.com" => ["token123"]},
        cname: %{"blog.example.com" => ["dijkstra-ledger.fly.dev"]}
      }
  """
  @behaviour Ledger.CustomDomains.DnsResolver
