defmodule LedgerWeb.UnsubscribeControllerTest do
  ...
  test "a valid token opts the user out of onboarding emails", %{conn: conn, user: user} do
    ...
    refute Ledger.Repo.get!(User, user.id).wants_onboarding_emails
  end
  ...
  test "an invalid token changes nothing", %{conn: conn, user: user} do
    ...
    assert Ledger.Repo.get!(User, user.id).wants_onboarding_emails
  end
end
